...
// Fetch all player summaries
export const fetchSBAllOrgSummaries = async (): Promise<SBLeaderboardOrgSummary[]> => {
  const response = await axios.get(`${API_BASE_URL}/api/leaderboardsborgsummary/`);
  // Flatten if response is nested
  const rawPlayers = Array.isArray(response.data[0]) ? response.data.flat() : response.data;

  // Parse each player object
  let players: SBLeaderboardOrgSummary[] = rawPlayers
    .map((p: any) => ({
      ...p,
      first_id: p.first_id ? BigInt(p.first_id) : BigInt(0),
      total_kills: p.total_kills ? BigInt(p.total_kills) : BigInt(0),
    ...
    // Filter out empty player (no nickname and avg_rank 0)
    .filter((player: SBLeaderboardOrgSummary) => player.name && player.name.trim() !== "" && player.avg_rank !== 0);
...
// Fetch a player summary by nickname
export const fetchOrgSummaryByName = async (nickname: string): Promise<SBLeaderboardOrgSummary> => {
  const response = await axios.get<SBLeaderboardOrgSummary>(`${API_BASE_URL}/api/leaderboardsborgsummary/${encodeURIComponent(nickname)}`);
  return response.data;
};